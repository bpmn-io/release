 ...
 * Without it, the identifier is asked for, defaulting to `alpha`.
 *
 * @param {{ input?: NodeJS.ReadableStream, output?: NodeJS.WritableStream, preid?: string, defaultDistTag?: string }} [io]
 */
export function createInteractivePrompter({ input = process.stdin, output = process.stdout, preid: governedPreid, defaultDistTag } = {}) {
  ...
  return {
    async bump({ name, currentVersion }) {
      ...
      if (PRE_CHOICES.includes(type)) {
        ...
        // must never land on `latest` — so we still require a dist-tag.
        distTag = await askDistTag(preid);
      }

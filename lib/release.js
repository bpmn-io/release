...
export async function release(options = {}) {
  ...
  const rawExec = _exec ?? ((cmd, opts) => execSync(cmd, { cwd, encoding: 'utf-8', ...opts }));
  const exec = (cmd, opts = {}) => {
    const result = rawExec(cmd, opts);
  ...
  const getNpmDistTags = (name) => {
    try {
      return JSON.parse(exec(`npm view ${name} dist-tags --json`));
    } catch {
      return {};
    }
  };
  ...
  try {
    ...
    const infos = packages.map(({ dir, name, pkg }) => {
      ...
      if (!npmVersion) return { dir, name, pkg, npmVersion: null, currentVersion: '0.0.0' };

      // Use the highest published version across all dist-tags as the baseline.
      // This lets prerelease channels (alpha, beta, …) act as change-detection anchors
      // even though `npm view … version` only returns the `latest` dist-tag.
      const distTags = getNpmDistTags(name);
      let currentVersion = npmVersion;
      for (const [ , tagVersion ] of Object.entries(distTags)) {
        try {
          if (compareVersions(tagVersion, currentVersion) > 0) currentVersion = tagVersion;
        } catch { /* ignore unparseable dist-tag versions */ }
      }

      return { dir, name, pkg, npmVersion, currentVersion };
    });
  ...
  async function planIndependent(infos) {
    ...
    for (const info of infos) {
      // Use currentVersion (highest of stable + any prerelease dist-tag) as baseline.
      const tag = info.currentVersion !== '0.0.0' ? findPublishTag(info.name, info.currentVersion) : null;

      // Published but untagged — we can't detect its changes, so leave it out.
      if (info.currentVersion !== '0.0.0' && !tag) {
        logger.warn(`  warning: ${info.name}@${info.currentVersion} — no git tag found`);
        logger.warn(`  Create tag '${info.name}@${info.currentVersion}' to enable change detection.\n`);
        continue;
      }
    ...
    for (const info of changed) {
      ...
      const { type: bump, preid } = await prompter.bump({ name: info.name, currentVersion: info.currentVersion });
      if (bump === 'skip') continue;

      plan.push({ ...info, bump, preid, newVersion: bumpVersion(info.currentVersion, bump, preid) });
    }
  ...
  async function planFixed(infos) {
    ...
    const { type: bump, preid } = await prompter.bump({ name: 'all packages', currentVersion: sharedVersion });
    if (bump === 'skip') {
      return { plan: [], candidates: [] };
    }

    const newVersion = bumpVersion(sharedVersion, bump, preid);
    const plan = infos.map(info => ({ ...info, bump, preid, newVersion }));
  ...
  function executeRelease(strategy, plan, packages) {
    ...
    for (const { dir, name, newVersion } of plan) {
      ...
      // Publish. Prerelease versions get an explicit dist-tag derived from the
      // version identifier (e.g. `1.1.0-alpha.2` → `--tag alpha`) so they don't
      // overwrite the `latest` dist-tag. Stable versions use npm's default (`latest`).
      const preMatch = newVersion.match(/-([a-zA-Z][a-zA-Z0-9]*)\.\d+$/);
      const publishCmd = preMatch ? `npm publish --tag ${preMatch[1]}` : 'npm publish';
      exec(publishCmd, { cwd: join(cwd, dir), stdio: 'inherit' });

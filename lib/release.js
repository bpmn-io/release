...
export async function release(options = {}) {
  ...
  async function executeRelease(strategy, commitMessage, plan, packages) {
    ...
    for (const { dir, name, newVersion, distTag: tag, private: isPrivate } of plan) {
      logger.log(`\n${name}@${newVersion}`);

      // Build and test against the committed state before publishing/tagging.

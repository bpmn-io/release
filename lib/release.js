...
import { openGitHubReleaseDrafts } from './github.js';
 ...
 *   (default: true).
 * @param {boolean} [options.releaseDraft] after publishing, open a pre-filled
 *   GitHub release draft per release tag in the browser (`np`-style), when the
 *   `origin` remote is on GitHub. Best-effort: never fails the release
 *   (default: true).
 * @param {(url: string) => void} [options.open] browser opener used for the
 *   release drafts (default: `opener`).
...
export async function release(options = {}) {
  ...
  // Closing the prompter releases stdin, so it must happen before children
  // with inherited stdio run. Idempotent, so the `finally` below stays safe.
  ...
  // Like commitsSince, but with the short hash for the release draft body.
  // Called after the release commit exists, so it logs through `HEAD^` — the
  // release commit itself was not shipped. Without a baseline tag (first
  // release) the whole history up to `HEAD^` is the release.
  const commitsWithHashSince = async (tag, dir) => {
    const range = tag ? `${tag}..HEAD^` : 'HEAD^';
    const out = await run('git', [ 'log', range, '--pretty=format:%h %s', '--', dir ]);
    if (!out) return [];
    return out.split('\n').map(line => {
      const match = line.match(/^([0-9a-f]{7,40}) (.+)$/);
      return match ? { hash: match[1], subject: match[2] } : { subject: line };
    });
  };
  ...
  try {
    ...
    // A global dist-tag governs every published package. `latest` is never a
    // valid choice here: stable versions already default to it, and pre-releases
    // must never use it — so reject it up front, before any prompting or I/O,
    // rather than mid-run once a pre-release bump is chosen.
    if (distTag === LATEST_DIST_TAG) {
      throw new ReleaseError(
        `Refusing the global '${LATEST_DIST_TAG}' dist-tag. Stable versions default to it; ` +
        'pre-releases must use a non-latest tag (e.g. --dist-tag next).'
      );
    }
    ...
    const uncommittedFiles = await run('git', [ 'status', '--porcelain' ], { cwd });
    ...
    await run('npm', [ 'install' ], { cwd, stdio: 'inherit' });

    const staleLockfile = await run('git', [ 'status', '--porcelain' ], { cwd });
  ...
  async function planIndependent(infos) {
    ...
    for (const info of changed) {
      ...
      const decision = await prompter.bump({ name: info.name, currentVersion: info.currentVersion, publish: !info.private });
  ...
  async function planFixed(infos) {
    ...
    // A dist-tag is only meaningful when at least one package is published; an
    // all-private release publishes nothing, so don't ask for one.
    const willPublish = selected.some(({ info }) => !info.private);

    const decision = await prompter.bump({ name: 'all packages', currentVersion: sharedVersion, publish: willPublish });
    if (decision === 'skip') {
      return { plan: [], candidates: [] };
    }
    ...
    const plan = selected.map(({ info }) => ({ ...info, bump: type, preid, distTag, newVersion, previousTag: baselineTag }));
  ...
  async function executeRelease(strategy, commitMessage, plan, packages) {
    ...
    // `np`-style: open a GitHub release draft per published tag (private
    // packages are tagged but never published, so they get none).
    if (releaseDraft) {
      const drafts = [];
      if (strategy === 'fixed') {
        if (plan.some(p => !p.private)) {
          drafts.push({
            tag: tags[0],
            previousTag: plan[0].previousTag,
            commits: await commitsWithHashSince(plan[0].previousTag, '.'),
            prerelease: isPrerelease(plan[0].newVersion)
          });
        }
      } else {
        for (const p of plan) {
          if (p.private) continue;
          const tag = `${p.name}@${p.newVersion}`;
          drafts.push({
            tag,
            previousTag: p.tag,
            commits: await commitsWithHashSince(p.tag, p.dir),
            prerelease: isPrerelease(p.newVersion)
          });
        }
      }

      if (drafts.length) {
        logger.log('\nGitHub release draft(s):');
        await openGitHubReleaseDrafts({ run, logger, drafts, ...(open ? { open } : {}) });
      }
    }

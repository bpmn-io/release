...
export async function release(options = {}) {
  ...
  async function planFixed(infos) {
    ...
    const anyChange = !baselineTag || groups.some(g => g.commits.length > 0) || isPrerelease(sharedVersion);
    ...
    const plan = infos.map(info => ({ ...info, bump: type, preid, distTag, newVersion }));

 ...
 * packages are included by default (they will be versioned + tagged, but never
 * published to the registry); pass `excludePrivate` to leave them out of the
 * release entirely.
 *
 * @param {string} cwd repository root
 * @param {any} rootPkg parsed root package.json
 * @param {{ logger?: { warn: Function }, excludePrivate?: boolean }} [options]
 * @return {Array<{ dir: string, name: string, pkg: any }>}
 */
export function discoverPackages(cwd, rootPkg, options = {}) {
  const {
    logger = console,
  ...
  for (const pattern of patterns) {
    ...
    for (const rel of globSync(`${pattern}/package.json`, { cwd }).sort()) {
      ...
      // Private packages are versioned + tagged like any other, but never
      // published. Skip them entirely only when the caller opts out.
      if (pkg.private && excludePrivate) continue;
